...
public class ScheduleActivity extends AppCompatActivity {
    ...
    private void setupRecyclerView() {
        eventAdapter = new EventAdapter(eventList, event -> showAddOrUpdateEventDialog(event));
    ...
    private void loadEventsForDate(long date) {
        ...
        String[] selectionArgs = {String.valueOf(startOfDay.getTimeInMillis()), String.valueOf(endOfDay.getTimeInMillis())};
        ...
        if (cursor != null) {
            while (cursor.moveToNext()) {
                ...
                eventList.add(new Event(id, title, description, startDate));
            }
            cursor.close();
        }
    ...
    private void showAddOrUpdateEventDialog(final Event event) {
        ...
        builder.setPositiveButton(event != null ? "Atualizar" : "Adicionar", (dialog, which) -> {
            ...
            if (event != null) {
                updateEvent(event.getId(), title, description, isRecurring);
            } else {
                addEvent(title, description, selectedDate, isRecurring);
            }
        });
    ...
    private void addEvent(String title, String description, long startTime, boolean isRecurring) {
        long calID = 1;
    ...
    private void updateEvent(long eventId, String title, String description, boolean isRecurring) {
        ContentResolver cr = getContentResolver();
        ContentValues values = new ContentValues();
        values.put(CalendarContract.Events.TITLE, title);
        values.put(CalendarContract.Events.DESCRIPTION, description);
